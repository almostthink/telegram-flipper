...
class NotifyConfig(BaseModel):
    """Уведомления в Telegram через своего бота.
    ...
    покупке сразу идут цены коллекции по всем площадкам, а под ним —
    кнопка «Заморозить».

    Токен бота сюда не попадает: он лежит в хранилище секретов рядом с
    токенами площадок. Здесь только id чата — это не секрет.
    """

    enabled: bool = True
    #: Чат, куда слать. Заполняется сам, когда вы напишете боту /start.
    chat_id: int = 0
    on_buy: bool = True
    on_sell: bool = True
...
class CollectibleConfig(BaseModel):
    ...
    #: Надбавка за монохром — когда цвет модели совпадает с цветом фона.
    #: Сочетание ценится отдельно от редкости обеих частей: та же модель
    #: «в тон» и на случайном фоне стоит по-разному.
    monochrome_bonus: float = Field(default=0.20, ge=0, le=1.0)
    #: С какого совпадения цветов считать лот монохромным. Совпадение
    #: измеряется долей 0..1, а не «да/нет»: близкий, но заметно другой
    #: оттенок — это не монохром, но и не полная противоположность.
    min_monochrome_score: float = Field(default=0.6, ge=0, le=1)
